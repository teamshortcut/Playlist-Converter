...
def findSpotifyPlaylist(url):
    ...
    auth = util.oauth2.SpotifyClientCredentials(
    ...
    response = requests.get(
        "https://api.spotify.com/v1/playlists/" + playlistID, headers=headers
    )
...
def addSongsToGooglePlaylistFromInfo(playlist_id, track):
    ...
    for track in library:
        if (
            matchTitle(track["album"], albumName)
            and matchTitle(track["albumArtist"], artistName)
            and matchTitle(track["title"], trackName)
        ):
...
def spotifyToGoogle():
    ...
    if targetPlaylist == None:
        ...
        failures = (
            []
        )  # Stores the tracks that could not be found in Google Play library
        for i in playlistItems:  # add each track to Google Play playlist
            if not addSongsToGooglePlaylistFromInfo(
                playlist_id, i
...
def googleToSpotify():
    print(
        "Please enter the name of the Google Play Music playlist you would like to convert."
    )
    print(
    ...
    targetPlaylist = findGooglePlaylist(
        playlistName
    )  # Finds playlist matching inputted name
    ...
    if targetPlaylist == None:
        ...
        spotifyPlaylist = spotify.user_playlist_create(
            USERNAME, playlistName, public=public, description=playlistDescription
        )
        ...
        print(
            "Playlist converted: https://open.spotify.com/playlist/"
